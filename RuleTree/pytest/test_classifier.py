...
def test_classifier_iris():
    iris = load_iris()
    X = iris.data
    y = np.array(list(map(lambda x: iris.target_names[x], iris.target)))  # string
...
def test_classifier_target_as_int():
    iris = load_iris()
    X = iris.data
    y = iris.target  #np.array(list(map(lambda x: iris.target_names[x], iris.target))).astype(int)
