...
class PivotTreeStumpClassifier(DecisionTreeClassifier, RuleTreeBaseStump):
    ...
    def apply(self, X):
        X_transformed = self.pivot_split.transform(X[:, self.numerical], self.distance_measure)
...
class MultiplePivotTreeStumpClassifier(DecisionTreeClassifier, RuleTreeBaseStump):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.is_categorical = False
        self.is_oblique = False
        ...
        self.pivot_split = MultiplePivotSplitClassifier(**kwargs)
    ...
    def fit(self, X, y, distance_matrix, distance_measure, idx, sample_weight=None, check_input=True):
        ...
        if len(self.numerical) > 0:
            self.pivot_split.fit(X[:, self.numerical], y, distance_matrix, distance_measure, idx, sample_weight=sample_weight, check_input=check_input)
            X_transform = self.pivot_split.transform(X[:, self.numerical], distance_measure)
            candidate_names = self.pivot_split.get_best_tup_names()
            super().fit(X_transform, y)
